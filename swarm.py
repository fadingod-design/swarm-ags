...
"""
    ...
    def _quality_check(self, task: str) -> dict:
        ...
        # 3. 检查搜索是否执行
        if self._needs_search(task) and agent_id != "coordinator":
            # 检查是否有搜索结果标记
...
请按照质检标准进行评估，返回JSON格式的质检结果。"""
    ...
    def _needs_search(self, text: str) -> bool:
        ...
        keywords = [
            # Chinese keywords
            "最新", "新闻", "搜索", "查询", "天气", "当前", "目前",
            "实时", "行情", "动态", "今日", "价格", "股价", "汇率",
            "近期", "最新", "报道", "查找", "分析", "趋势",
            "怎么", "什么", "为什么", "是什么", "有哪些",
            # English keywords
            "current", "latest", "news", "search", "find", "today",
            "price", "market", "trend", "weather", "real-time",
            "recent", "now", "update", "data", "report", "analysis",
